const TTL = {
  ...
  streams:     5 * 60 * 1000,  // 5 min
  seriesInfo: 30 * 60 * 1000,  // 30 min
};
...
class IPTVApi {
  ...
  getLiveStreams() {
    return this._cached("live_streams", TTL.streams, () =>
      this.fetch(this.buildUrl("get_live_streams"))
    );
  }

  getVODCategories() {
    return this._cached("vod_categories", TTL.categories, () =>
      this.fetch(this.buildUrl("get_vod_categories"))
    );
  }
  ...
  getAllVODStreams() {
    return this._cached("vod_streams_all", TTL.streams, () =>
      this.fetch(this.buildUrl("get_vod_streams"))
    );
  }

  getSeriesCategories() {
    return this._cached("series_categories", TTL.categories, () =>
      this.fetch(this.buildUrl("get_series_categories"))
    );
  }
  ...
  getAllSeries() {
    return this._cached("series_all", TTL.streams, () =>
      this.fetch(this.buildUrl("get_series"))
    );
  }
